...
int main()
{
    ...
    std::cout << std::endl << ">> d1 has only columns x,c" << std::endl ;
    RooDataSet* d1 = (RooDataSet*) d.reduce(RooArgSet(x,c)) ;
    d1->Print("v") ;
    
    std::cout << std::endl << ">> d2 has only column y" << std::endl ;
    RooDataSet* d2 = (RooDataSet*) d.reduce(RooArgSet(y)) ;
    d2->Print("v") ;
    
    std::cout << std::endl << ">> d3 has only the points with y>5.17" << std::endl ;
    RooDataSet* d3 = (RooDataSet*) d.reduce("y>5.17") ;
    d3->Print("v") ;
    
    std::cout << std::endl << ">> d4 has only columns x,c for data points with y>5.17" << std::endl ;
    RooDataSet* d4 = (RooDataSet*) d.reduce(RooArgSet(x,c),"y>5.17") ;
    d4->Print("v") ;
    
    // The merge() function adds two data set column-wise
    std::cout << std::endl << ">> merge d2(y) with d1(x,c) to form d1(x,c,y)" << std::endl ;
    d1->merge(d2) ;
    d1->Print("v") ;
    
    // The append() function addes two datasets row-wise
    std::cout << std::endl << ">> append data points of d3 to d1" << std::endl ;
    d1->append(*d3) ;
    d1->Print("v") ;
    ...
    std::cout << ">> construct dh (binned) from d(unbinned) but only take the x and y dimensions," << std::endl
    << ">> the category 'c' will be projected in the filling process" << std::endl ;
    ...
    std::cout << ">> number of bins in dh   : " << dh.numEntries() << std::endl ;
    std::cout << ">> sum of weights in dh   : " << dh.sum(kFALSE)  << std::endl ;
    std::cout << ">> integral over histogram: " << dh.sum(kTRUE)   << std::endl ; // accounts for bin volume
    
    // Locate a bin from a set of coordinates and retrieve its properties
    x = 0.3 ;  y = 20.5 ;
    std::cout << ">> retrieving the properties of the bin enclosing coordinate (x,y) = (0.3,20.5) " << std::endl ;
    std::cout << " bin center:" << std::endl ;
    dh.get(RooArgSet(x,y))->Print("v") ; // load bin center coordinates in internal buffer
    std::cout << " weight = " << dh.weight() << std::endl ; // return weight of last loaded coordinates
    ...
    std::cout << ">> Creating 1-dimensional projection on y of dh for bins with x>0" << std::endl ;
    ...
    std::cout << std::endl << ">> Persisting d via ROOT I/O" << std::endl ;
